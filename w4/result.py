...
class PaddingOracle(object):
	...
	def query(self, q):
		target = self.targetURL + urllib.parse.quote(q)    # Create query URL
...
class CharGuesser(object):
	...
	def guessPrecedingChar(self, currentChar=None):
		...
		for char in [chr(x) for x in range(0,256)]:
			if not self.checkUsed(char):
				return self.setUsed(char)	
...
def main():
	cryptoText = 0xf20bdba6ff29eed7b046d1df9fb7000058b1ffb4210a580f748b4ac714c001bd4a61044426fb515dad3f21f18aa577c0bdf302936266926ff37dbf7035d5eeb1
	blockSize = 16 # 128 bit AES
	cryptoBlocks  = splitCount(cryptoText, blockSize) # x 16 byte blocks, where block 0 is the IV
	messageBlocks = splitCount(("00" * blockSize * (len(cryptoBlocks)-1)),  blockSize) # init message blocks
	...
	for blockNum in reversed(list(range(0,len(cryptoBlocks)))):


		# loop all 16 positions, last to first
		for position in reversed(list(range(0, blockSize))):
			...
			while counter < 10:
				...
				print(guess)
				print(paddingNum)
				# print(type(cryptoSourceBlock))
				print(cryptoSourceBlock.encode())

				# XOR the cryptoblock postion with the guess and the paddingnum
				# cryptoSourceBlock[position] ^= ord(guess) ^ paddingNum
				cryptoSourceBlock[position]  = str(int(cryptoSourceBlock[position]) ^ ord(guess) ^ paddingNum)

				cryptoGuess = buildCryptoString(str(cryptoText), (blockNum * int(blockSize)))
...
def buildCryptoString(cryptoText: str ,  position: int):
